...
4. Stores the Shortcut story ID, URL, and workflow state (name plus its `backlog` / `unstarted` / `started` / `done` type) on the report. The list and edit views colour the state green when it is done and amber while it is started, so open tickets stand out. The list view does not link to the Payload document. The Shortcut URL is shown as a "Shortcut link" so people open the ticket there.
